...
from api.dependencies.amms import RiderBearerToken, RideHandler
...
async def get_ride(rde_id: UUID, bearer_token: RiderBearerToken, handler: RideHandler):
    try:
...
async def assign_pod(rde_id: UUID, request: RideAssignRequest, handler: RideHandler):
...
async def board_ride(rde_id: UUID, handler: RideHandler):
    try:
...
async def complete_ride(rde_id: UUID, request: RideCompleteRequest, handler: RideHandler):
...
async def cancel_ride(rde_id: UUID, bearer_token: RiderBearerToken, handler: RideHandler):
    try:
